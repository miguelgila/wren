...
///
/// Always includes:
/// - `BUBO_JOB_NAME`, `BUBO_NUM_NODES`, `BUBO_TOTAL_RANKS`, `BUBO_HOSTFILE`
///
...
pub fn bare_metal_env_vars(
    ...
    env.insert("BUBO_JOB_NAME".to_string(), job_name.to_string());
    env.insert("BUBO_NUM_NODES".to_string(), nodes.len().to_string());
    env.insert("BUBO_TOTAL_RANKS".to_string(), total.to_string());
    env.insert("BUBO_HOSTFILE".to_string(), hostfile_path.to_string());
...
mod tests {
    ...
    fn make_cray_spec(nodes: u32, tasks_per_node: u32, iface: Option<&str>) -> MPIJobSpec {
        MPIJobSpec {
            ...
            nodes,
            tasks_per_node,
            backend: bubo_core::ExecutionBackendType::Reaper,
            container: None,
            reaper: None,
            mpi: Some(MPISpec {
    ...
    fn test_bare_metal_mpirun_args_cray_mpich() {
        ...
        let args = bare_metal_mpirun_args(&spec, "/etc/bubo/hostfile");
        ...
        assert!(args.contains(&"--hostfile".to_string()));
        assert!(args.contains(&"/etc/bubo/hostfile".to_string()));
    ...
    fn test_bare_metal_env_vars_common_fields() {
        ...
        let env = bare_metal_env_vars("my-job", &spec, &nodes, "/etc/bubo/hostfile");
        assert_eq!(env["BUBO_JOB_NAME"], "my-job");
        assert_eq!(env["BUBO_NUM_NODES"], "2");
        assert_eq!(env["BUBO_TOTAL_RANKS"], "8");
        assert_eq!(env["BUBO_HOSTFILE"], "/etc/bubo/hostfile");
    }
    ...
    fn test_srun_args_basic() {
        let spec = make_cray_spec(4, 8, None);
        let args = srun_args(&spec, "/etc/bubo/hostfile");
        assert_eq!(args[0], "srun");
        assert!(args.contains(&"--nodes".to_string()));
        assert!(args.contains(&"4".to_string()));
        assert!(args.contains(&"--ntasks-per-node".to_string()));
