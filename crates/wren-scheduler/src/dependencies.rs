...
impl DependencyResolver {
    ...
    /// `Err(WrenError::ValidationError)` if a cycle is detected, including
    /// the names of the jobs involved.
    pub fn validate_no_cycles(&self) -> Result<(), WrenError> {
...
impl JobArraySpec {
    ...
    /// ```
    pub fn parse(input: &str) -> Result<Self, WrenError> {
        ...
        let start: u32 = start_str.parse().map_err(|_| WrenError::ValidationError {
            reason: format!(
        ...
        let end: u32 = end_str.parse().map_err(|_| WrenError::ValidationError {
            reason: format!(
